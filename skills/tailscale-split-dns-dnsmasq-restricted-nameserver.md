...
tags: []
history-source: "https://github.com/HomericIntelligence/Mnemosyne/blob/ed1bd4f54fd4aaba446af92c8b3ab9aff2589ae3/skills/tailscale-split-dns-dnsmasq-restricted-nameserver.history"
...
Replace the quoted placeholder with the service host's actual tailnet address
before using these commands. Use the same address in the dnsmasq configuration.
...
curl -k --resolve "svc.example.com:443:${TAILNET_IP}" https://svc.example.com/status.php

# 1. dnsmasq config bound to override just the one hostname, forward everything else.
#    /etc/tailscale-dns/dnsmasq.conf :
#      address=/svc.example.com/<tailnet-ip>
#      no-resolv
#      server=1.1.1.1
#      server=8.8.8.8
...
docker run -d --name tailscale-dns --restart unless-stopped \
  -p "${TAILNET_IP}:53:53/udp" -p "${TAILNET_IP}:53:53/tcp" \
...
# 3. Tailscale admin -> DNS -> Nameservers -> Add nameserver <tailnet-ip>
#    -> toggle "Restrict to domain" -> example.com

# 4. Verify (see Detailed Steps below):
dig "@${TAILNET_IP}" svc.example.com +short          # -> <tailnet-ip> (override)
dig "@${TAILNET_IP}" other.example.com +short        # -> real public IP (forwarded)
dig svc.example.com +short                          # via MagicDNS -> <tailnet-ip>
```
...
1. **Confirm the service answers on the tailnet IP first.** Before touching DNS, prove the
   ...
   hostname: `curl -k --resolve "svc.example.com:443:${TAILNET_IP}" https://svc.example.com/status.php`.
...
2. **Write the dnsmasq config** at `/etc/tailscale-dns/dnsmasq.conf`:
   - `address=/svc.example.com/<tailnet-ip>` — the override (add one `address=` line per
     host-served service).
...
3. **Run dnsmasq in Docker bound ONLY to the host's tailnet IP on port 53** (UDP + TCP). Binding
   to `<tailnet-ip>:53` rather than `0.0.0.0:53` keeps it off the LAN/WAN interfaces so it is
...
4. **Add the restricted nameserver in the Tailscale admin console**: DNS -> Nameservers ->
   Add nameserver `<tailnet-ip>` -> toggle **Restrict to domain** -> `example.com`. Now tailnet
...
5. **Verify** (all four should hold):
   - `dig "@${TAILNET_IP}" svc.example.com +short` -> `<tailnet-ip>` (override served locally).
   - `dig "@${TAILNET_IP}" other.example.com +short` -> that name's real public IP (forwarded upstream).
   - After the restricted nameserver is live, the host's own default resolver
     ...
     `dig svc.example.com +short` -> `<tailnet-ip>`.
   - A real upload via the resolved name is LAN-fast. Isolate the two paths with `curl --resolve`
     to compare: `--resolve "svc.example.com:443:${TAILNET_IP}"` (tailnet) vs
     `--resolve svc.example.com:443:<public-wan-ip>` (hairpin).
6. **To add more services**, append another `address=/svc2.example.com/<tailnet-ip>` line and
...
| 3 | (Design consideration) binding dnsmasq to `0.0.0.0:53` | Would expose an open resolver on LAN/WAN and could collide with other port-53 listeners | Bind ONLY to the tailnet IP (`<tailnet-ip>:53`) so it is reachable solely over Tailscale and is not an open resolver |
...
| Tailnet (this fix) | `curl --resolve "svc.example.com:443:${TAILNET_IP}"` | ~30 MB/s (LAN speed) |
...
Use `TAILNET_IP` from the Quick Reference, set to the actual tailnet address.
...
**Tailscale admin steps:** DNS -> Nameservers -> Add nameserver `<tailnet-ip>` -> toggle
**Restrict to domain** -> enter `example.com` -> Save.

**Gotchas / operational notes:**

- The `-p <tailnet-ip>:53` bind is on the `tailscale0` interface, so on reboot the container
