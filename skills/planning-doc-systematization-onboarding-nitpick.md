...
description: "Plan documentation conventions and onboarding improvements from current contents, consumers, and project scope; avoid unsupported cleanup or implementation expansion."
...
history-source: "https://github.com/HomericIntelligence/Mnemosyne/blob/1956c91d76867bc2e484eaf573a57051855186f8/skills/planning-doc-systematization-onboarding-nitpick.history"
...
4. **Prefer an onboarding index over duplicated instructions.** CONTRIBUTING.md already had every step (Development Setup, Testing, PR Process); the only gap was sequencing. Link existing canonical steps when they suffice. Add concise context or a command example when it materially improves onboarding, while keeping its maintenance source clear.
...
2. **Over-generalized README header keys — RESOLVED (framing).** The three keys are now presented as "the existing example uses these keys … a convention by example, not a hard schema"; the narrative sections are useful guidance, not a new schema.
