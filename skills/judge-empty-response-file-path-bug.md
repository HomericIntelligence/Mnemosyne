...
After fixing the code, already-completed subtests have zero-score is_valid=False results baked into the checkpoint. Reset run states to re-run the judge stage:

```python
import json
from pathlib import Path

cp_path = Path('/path/to/checkpoint.json')
cp = json.loads(cp_path.read_text())
base = cp_path.parent

# Back up first!
import shutil
shutil.copy(cp_path, str(cp_path) + '.bak')

# Find subtests with ALL invalid judges
subtests_to_reset = []
for st in range(24):
    st_id = f'{st:02d}'
    all_invalid, has_runs = True, False
    for run in range(1, 4):
        rf = base / 'T0' / st_id / f'run_{run:02d}' / 'run_result.json'
        if rf.exists():
            has_runs = True
            for j in json.loads(rf.read_text()).get('judges', []):
                if j.get('is_valid', False):
                    all_invalid = False
                    break
            if not all_invalid:
                break
    if has_runs and all_invalid:
        subtests_to_reset.append(st_id)

# Reset states
for st_id in subtests_to_reset:
    cp['subtest_states']['T0'][st_id] = 'pending'
    for run_id in cp['run_states']['T0'][st_id]:
        cp['run_states']['T0'][st_id][run_id] = 'judge_prompt_built'

cp['tier_states']['T0'] = 'config_loaded'
if cp.get('experiment_state') == 'complete':
    cp['experiment_state'] = 'tiers_dispatched'
...
| Item | Count |
|------|-------|
| Subtests reset to pending | 15 (T0/00-11, T0/16-18) |
| Runs reset to judge_prompt_built | 45 |
| Subtests preserved (valid judges) | 3 (T0/12-15) |
