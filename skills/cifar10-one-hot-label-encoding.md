...
tags:
  ...
  - training
  - mojo
history-source: "https://github.com/HomericIntelligence/Mnemosyne/blob/1956c91d76867bc2e484eaf573a57051855186f8/skills/cifar10-one-hot-label-encoding.history"
...
| --- | --- | --- | --- |
| Raw integer labels | Passed `(B,)` integer class indices to cross-entropy | The loss expects `(B, num_classes)` float32 target distributions | Convert class indices to one-hot targets before loss computation |
| Float casts | Cast `(B,)` integer labels to float32 | The cast changes the type but does not create the required class dimension | Create a `(B, num_classes)` one-hot tensor instead of only changing the data type |
| Soft targets | Used `0.7` for the true class and distributed `0.3` across other classes | The targets added unintended label smoothing instead of standard CIFAR-10 hard targets | Use `1.0` for the true class and `0.0` for the other classes |
