...
## Sub-Pattern: Counter-Flatness Boundary Test for a Metrics-Guarded Eviction Path

> **Warning:** This sub-pattern (added in v1.12.0, from ProjectHermes issue #697 plan) is **unverified** — it is a PLAN, the code was NOT executed or CI-validated. Treat as a hypothesis until CI confirms. Earlier verified-local material retains its stated levels.

This applies the skill's bidirectional principle to a **metrics counter** rather than a structural mirror. The #697 case: ProjectHermes's in-memory dead-letter deque (`deque(maxlen=N)`) increments `hermes_dead_letter_evictions_total` when a full deque evicts its oldest entry, guarded by a `was_full` pre-append check (`publisher.py:393-401`). The existing suite asserted only the positive direction (the N+1th insert increments the counter by 1). The plan adds ONE test asserting the counter stays FLAT when the deque fills exactly to capacity (N inserts into `maxlen=N`) without overflowing — locking the guard's boundary condition.

### Proposed Workflow

> **Warning:** This workflow has not been validated end-to-end. Treat as a hypothesis until CI confirms.

1. **The negative/boundary direction of a counter guard is the un-asserted drift surface.** When a metric increments on an eviction/overflow event, the existing suite typically covers only the positive case (Nth+1 insert increments by 1). Add a dedicated boundary test: fill EXACTLY to capacity (N inserts into `maxlen=N`) and assert the counter is UNCHANGED. This catches an off-by-one in a `was_full = len(dq) == maxlen` pre-append guard that the positive test alone cannot catch. Same bidirectional principle as the mirror-invariant sub-pattern above, applied to a metrics counter.

2. **Assert delta, never absolute, on a session-global Prometheus counter.** Module-level counters in `prometheus_client.REGISTRY` are shared across the whole test session; `assert after == 0.0` is order-dependent (any earlier test that triggered an eviction breaks it). Use `before = REGISTRY.get_sample_value("<metric>_total") or 0.0` / run the fill / `assert after == before` — the `or 0.0` handles the counter-not-yet-created case. Mirror the sibling positive test's EXACT read pattern so both directions share semantics.

3. **Pair the flatness assertion with a landed-state assertion.** Also assert the queue actually reached capacity (e.g. `dead_letter_count == 2`) so the counter being flat is proven to mean "no eviction", not "publishes silently failed" — otherwise the test passes vacuously on a broken publish path.

4. **Mutation self-falsification check for a flatness test.** A test asserting "nothing happened" can pass vacuously. Verify it can fail: temporarily mutate the guard (`if was_full:` → `if True:`), confirm the new test FAILS, revert. Run in a subshell / never commit the mutation (the tree-mutation hygiene rule from the v1.2.0 coverage-altitude lesson).

5. **Planning honesty — uncertain assumptions to hand the reviewer:**
   - The plan relied on the sibling tests' fixture (mocked `_js`, monkeypatched `get_settings` giving `dead_letter_max_size=2`) actually driving the dead-letter path for unknown events — inferred from READING sibling tests, not re-executed.
   - Cited line numbers (`tests/test_dead_letter_limits.py:486`, `publisher.py:393-401`) were read once and may drift — re-grep at implementation time.
   - The reused fixture monkeypatches `hermes.config.get_settings` at module attribute level — if `publisher.publish` imports it differently, the `maxlen=2` assumption breaks silently, which the `dead_letter_count == 2` landed-state assertion is designed to expose.
...
| ProjectHermes | issue #697 plan (PLAN — **unverified**) | counter-flatness boundary sub-pattern: one test asserting `hermes_dead_letter_evictions_total` stays FLAT when the `deque(maxlen=N)` fills exactly to capacity without overflowing, locking the `was_full` pre-append guard (`publisher.py:393-401`); delta-based Prometheus assertions (`before = REGISTRY.get_sample_value(...) or 0.0` / `assert after == before`) mirroring the sibling positive test; paired landed-state assertion (`dead_letter_count == 2`) so flatness cannot pass vacuously; mutation self-falsification (`if was_full:` → `if True:` must FAIL the test, then revert) |

## Tags

`#executable-convention` `#invariant-guard` `#exit-code-collision` `#fail-safe` `#read-only-verification` `#log-line-anchoring` `#observability` `#cli-verify-mode` `#ci-gate` `#bidirectional-invariant` `#mirror-parity` `#allowlist-with-rationale` `#test-structure` `#commit-trailer` `#dco-signoff` `#precommit-hook` `#sibling-checker-mirror` `#coverage-altitude` `#negative-branch-test` `#argparse-default-branch` `#nogo-tightening` `#inferred-data-honesty` `#doc-table-code-parity` `#membership-only-guard` `#guard-scope-population` `#partial-compliance` `#scope-lock-test` `#ghost-directory-guard` `#prefiltered-population-fixture` `#install-smoke-test` `#self-test-in-guard` `#imported-location` `#find-package` `#canonical-check-name` `#ecosystem-ci-board` `#manifest-repo-install` `#check-run-on-main` `#real-check-vs-na` `#canonical-check-run` `#release-packaging` `#least-privilege-workflow` `#sigpipe-safe` `#action-pin-verification` `#dry-run-release-check` `#trusted-publishing-oidc` `#pipeline-validator` `#counter-flatness` `#prometheus-delta-assertion` `#boundary-test` `#eviction-metrics` `#vacuous-pass-guard` `#projectscylla` `#myrmidons` `#hephaestus` `#hermes`
