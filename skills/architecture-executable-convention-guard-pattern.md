...
tags:
  ...
  - find-package
  - hephaestus
  - canonical-install-check
  - package-prerequisite
  - flat-layout-packaging
  - no-checkout-install-smoke
  - cross-repo-action-pin
---
...
## Sub-Pattern: Canonical Python `install` Check Blocked on a Missing `package` Job (planning instance: ProjectMnemosyne #2912)

> Verification level: **mixed** — the packaging-config prototype (steps 2–3 below) was EXECUTED locally end-to-end and is **verified-local**; the CI YAML (`package`/`install` jobs) is a PLAN only and **unverified** until the PR's first CI run confirms it.

This applies the pattern to the ecosystem's canonical `install` check-run for a *Python* repo whose `package` prerequisite does not exist yet (the exact "blocked on `package`" situation the CI-naming board issues describe). The durable lessons are about how to plan an executable gate whose PREDECESSOR gate is missing:

1. **Grep-prove the predecessor is absent, then bundle the minimal prerequisite in the SAME PR — with an explicit rebase-and-drop contingency.** `grep -rn 'name: package' .github/workflows/` → no matches proved no `package` job existed. A standalone `install` PR would be unmergeable (`needs: package` can never be satisfied), and a self-building `install` job would fabricate the convention's package→install artifact contract. The plan therefore bundles the minimal prerequisite (pyproject `[build-system]` + a `package` job that builds, `twine check`s, and uploads `dist/`) and adds an implementation-order step: re-grep against latest `main` immediately before opening the PR; if a sibling `package` PR landed meanwhile, rebase, drop the duplicated hunk, and point `needs:`/the artifact name at the landed job. This is the "a captured plan is not a merged artifact" rule turned into a concrete race-handling step.

2. **Flat-layout repo: declare the distributable explicitly — and PROTOTYPE the packaging config in a throwaway dir before the plan promises it.** Multiple top-level dirs (`scripts/`, `skills/`, `tests/`, `templates/`, ...) break setuptools auto-discovery. `package-dir = {"" = "scripts"}` + `py-modules = ["skill_utils"]` ships the one shared library module without restructuring the repo's flat `from skill_utils import ...` imports. This exact config was executed in `/tmp` (testability-precondition rule applied to a packaging config): `python -m build` produced the wheel + sdist, and a clean-venv `pip install dist/*.whl` imported the module from `site-packages` and round-tripped `parse_frontmatter()`.

3. **Install-smoke job with NO checkout step, plus an in-test provenance assertion.** Omitting `actions/checkout` guarantees the smoke import cannot be satisfied from the repo tree (there is no tree); belt-and-braces, the smoke script also asserts `"site-packages" in inspect.getsourcefile(module)` so a future checkout-adding edit cannot silently turn the smoke vacuous. This is the Python analogue of the clean-staging-prefix rule in the ProjectKeystone instance above.

4. **A cross-repo action-pin pairing is evidence, not verification.** The `actions/download-artifact` v6.0.0 SHA was taken from sibling ProjectHermes, where it demonstrably pairs with the same `upload-artifact` v7.0.1 pin this repo already uses. That pairing is verified only in the SIBLING repo — name it in the plan's risk list so the reviewer and the PR's first CI run watch it, instead of presenting it as settled.

5. **"No local caller" for a `workflow_call` file is in-repo evidence only.** The repo's `_checks.yml` (`on: workflow_call`) has no caller anywhere in `.github/`, the justfile, or `pixi.toml`, and the newest canonical-check precedent (the `test` aggregate, PR #2914) went to `_required.yml` only — so the new jobs follow that precedent. But a `workflow_call` file can be invoked from OTHER repos, which grep cannot see; record the mirror-gap decision and its evidence explicitly rather than claiming the file is dead.

6. **A wheel-vs-bundle route choice inherits ANOTHER issue's intent — state the evidence, expect the objection.** The issue sanctioned "wheel/sdist (or marketplace bundle)"; the wheel route was chosen because the sibling `package`-gap issue's own diagnosis (missing `[build-system]`, flat-layout auto-discovery failure) implies the wheel is its intent, and because it mirrors the Hermes-verified shape with zero custom bundle code. A reviewer preferring the marketplace-bundle route is a legitimate objection; the plan must show the decision's evidence, not just the decision.
...
| Scoped the PR to only the issue-named `install` job (ProjectMnemosyne #2912, PLAN) | First instinct: add just the `install` job, since the issue names only `install` and `package` is a separate gap issue | With no `package` job in any workflow (grep-proved), `needs: package` can never be satisfied — the check is permanently red and the PR unmergeable; a self-building `install` job instead would fabricate the convention's package→install artifact contract | When the predecessor gate is missing, bundle the MINIMAL prerequisite in the same PR and write an explicit re-grep + rebase-and-drop contingency step for the sibling-PR race |
| Nearly promised the flat-layout packaging config unexecuted (ProjectMnemosyne #2912) | `package-dir = {"" = "scripts"}` + `py-modules = ["skill_utils"]` "looks right" in prose and could have shipped in the plan untested | setuptools flat-layout auto-discovery errors on multiple top-level dirs, and the `package-dir`/`py-modules` interaction is exactly the kind of config that fails in non-obvious ways | Prototype the packaging config end-to-end in a throwaway dir (build wheel+sdist, clean-venv install, site-packages import assert) BEFORE the plan promises the CI job — the testability-precondition rule applies to packaging configs, not just tests |
| Presented a sibling repo's action pin as settled (ProjectMnemosyne #2912, PLAN) | Took the `download-artifact` v6.0.0 SHA from ProjectHermes because it pairs there with the same `upload-artifact` v7.0.1 pin | Cross-repo pairing is evidence, not verification — upload-v7↔download-v6 compatibility in THIS repo is an assumption until the PR's first CI run | Name every cross-repo-sourced pin in the plan's explicit risk list so the reviewer and the first CI run watch it |
...
### Canonical-install-blocked-on-package note (v1.9.0 — packaging prototype verified-local; CI YAML plan-only)

Copy-paste flat-layout packaging config (executed end-to-end in a throwaway dir: wheel + sdist built, clean-venv installed, imported from `site-packages`):
...
Provenance-asserting install smoke (run with NO repo checkout):

```python
import inspect, skill_utils
...
| ProjectMnemosyne | issue #2912 (PLAN — packaging prototype **verified-local**; CI YAML unverified) | canonical-install-blocked-on-package sub-pattern: grep-proved no `package` job exists anywhere in `.github/workflows/`; plan bundles the minimal prerequisite — pyproject `[build-system]` + `package-dir={"": "scripts"}`/`py-modules=["skill_utils"]` prototyped end-to-end in `/tmp` (wheel+sdist built, clean-venv `pip install`, site-packages import + `parse_frontmatter` round-trip) — plus Hermes-mirrored `package`→`install` jobs with SHA-pinned actions; `install` job has NO checkout step and an `inspect.getsourcefile` site-packages assertion; explicit rebase-and-drop contingency for the sibling `package`-gap PR race; risks named for the reviewer: cross-repo `download-artifact` v6 pin, `_checks.yml` no-local-caller inference (workflow_call is externally callable), wheel-vs-bundle route inheriting the sibling issue's intent |

## Tags

`#executable-convention` `#invariant-guard` `#exit-code-collision` `#fail-safe` `#read-only-verification` `#log-line-anchoring` `#observability` `#cli-verify-mode` `#ci-gate` `#bidirectional-invariant` `#mirror-parity` `#allowlist-with-rationale` `#test-structure` `#commit-trailer` `#dco-signoff` `#precommit-hook` `#sibling-checker-mirror` `#coverage-altitude` `#negative-branch-test` `#argparse-default-branch` `#nogo-tightening` `#inferred-data-honesty` `#doc-table-code-parity` `#membership-only-guard` `#guard-scope-population` `#partial-compliance` `#scope-lock-test` `#ghost-directory-guard` `#prefiltered-population-fixture` `#install-smoke-test` `#self-test-in-guard` `#imported-location` `#find-package` `#hephaestus` `#canonical-install-check` `#package-prerequisite` `#flat-layout-packaging` `#no-checkout-install-smoke` `#cross-repo-action-pin`
