...
| **Verification** | verified-ci (function-local / version-guard / Windows-guard / lazy-exports); verified-local (surface-pin-stale fix, access-time deprecation-warning implementation — section F executed end-to-end with all tests passing locally; CI pending after ProjectHephaestus PR merges); **unverified** planning-guidance (section G — promoting an undeclared public-looking symbol to the stable surface; derived from plans for ProjectHephaestus issues #1513 and #1511 (§G′ second instance), neither executed in CI; section H — adding the companion `__dir__()` to a PEP 562 lazy-loader package; derived from a plan for ProjectHephaestus issue #1512, not yet executed in CI) |
...
##### G′. Second instance of the §G pattern — `hephaestus.cli` package-vs-submodule `__all__` gap (UNVERIFIED, issue #1511)

> **Verification:** `unverified`. A SECOND planning instance of the §G pattern, derived from an implementation *plan* for ProjectHephaestus issue #1511 — a POLA/ISP `[S14 API Design]` audit finding that symbols present in `hephaestus/cli/utils.__all__` are absent from the package-level `hephaestus/cli/__init__.__all__`. The plan re-exports the missing symbols from the package `__init__.py`, adds them to `__all__`, and strengthens the existing surface test. The plan has **not** been executed in CI; treat every step below as a checklist to confirm, not a verified outcome. These three learnings EXTEND the five §G checks above; they do not replace them.

1. **Diff the two `__all__` lists yourself — the audit's named symbols can be INCOMPLETE.** Do NOT trust the issue's enumerated symbols as the full gap. Compute the set difference between the submodule's `__all__` and the package's `__all__` on disk. Issue #1511 named only `add_dry_run_arg` and `DRY_RUN_HELP_CAVEAT`, but the on-disk difference revealed FOUR missing symbols (also `add_github_throttle_args`, `configure_github_throttle_from_args`), all missing for the identical POLA/ISP reason. Fixing only the issue-named subset leaves the same defect class and re-triggers the audit. When the finding is "module `__all__` ⊋ package `__all__`", fix the WHOLE gap.

   ```bash
   # Enumerate the submodule's declared surface and compare against the package's
   python3 -c "from hephaestus.cli import utils; print(sorted(utils.__all__))"
   python3 -c "import hephaestus.cli as pkg; print(sorted(pkg.__all__))"
   # Or directly: the symbols to promote are exactly this non-empty set
   python3 -c "from hephaestus.cli import utils; import hephaestus.cli as pkg; print(sorted(set(utils.__all__) - set(pkg.__all__)))"
   ```

2. **The target package may have NO COMPATIBILITY.md per-package symbol table — so §G step 1 (version-stamping the "Added" column) can be entirely N/A.** Before agonizing over the next-release version for the "Added" column, grep for an actual per-package symbol table; it may not exist. For `hephaestus.cli`, the per-package symbol tables in `COMPATIBILITY.md` exist only for `logging`/`config`/`io`/`utils`. `hephaestus.cli` is listed as Stable in the tier list (`COMPATIBILITY.md:53`) but has no symbol table to update — so the version-stamp step is skipped, and the promoted symbols are covered by the package-level Stable guarantee via their presence in `__all__`.

   ```bash
   grep -n '### `hephaestus.cli`' COMPATIBILITY.md   # if NO hit, there is no table to version-stamp
   grep -n '### `hephaestus' COMPATIBILITY.md         # see which packages actually have a symbol table
   ```

3. **A green EXISTING surface test may be a WEAK membership spot-check that does not catch the gap — strengthen it to a full set-difference pin.** A surface test that only asserts a hand-picked few symbols are `in package.__all__` will pass even while the package surface is incomplete. `tests/unit/cli/test_utils.py::test_cli_all_lists_framework` (around lines 348–361) asserted only 5 named symbols were `in cli.__all__`, so it stayed green despite the four-symbol gap. Replace/augment it with a full set-difference pin (`set(utils.__all__) - set(cli.__all__)` asserted empty, per §B step 8) PLUS a re-export IDENTITY check (`cli.add_dry_run_arg is utils.add_dry_run_arg`, per §G step 4). A green surface test does NOT prove the surface is complete if it only spot-checks membership.

   ```python
   def test_cli_all_is_superset_of_utils_all() -> None:
       import hephaestus.cli as cli
       from hephaestus.cli import utils
       missing = set(utils.__all__) - set(cli.__all__)
       assert not missing, f"hephaestus.cli.__all__ missing utils framework symbols: {sorted(missing)}"
       # identity: the package-root re-export must be the SAME object as the submodule's
       assert cli.add_dry_run_arg is utils.add_dry_run_arg
   ```

**Reviewer risk callouts (issue #1511 plan):**

- *Most uncertain assumption:* that re-exporting the two *throttle* symbols (`add_github_throttle_args`, `configure_github_throttle_from_args`) is in-scope. It is defensible — they are the same defect class as the issue-named symbols — but a reviewer could argue it is scope-creep beyond the issue. The plan justifies it as: fixing only the named subset re-triggers the same audit finding.
- *Relied on without direct execution:* the claim that widening `cli/__init__.py` does not regress `test_import_surface.py` / `test_automation_boundary.py`. This was reasoned from `utils.py` importing only stdlib + `hephaestus._version_lookup`, but was **NOT run** — the verification commands (§G step 5) are listed but unexecuted. This is an unverified plan.
...
| ProjectHephaestus | Issue #1511 — `[S14 API Design]` POLA/ISP promote `hephaestus/cli/utils.__all__` symbols to `hephaestus/cli/__init__.__all__` | **PLAN ONLY / unverified** (section G′ — second instance of the §G pattern). Plan promotes the package-vs-submodule `__all__` gap. Three new learnings captured: (1) diff the two `__all__` lists yourself — the audit named only `add_dry_run_arg` + `DRY_RUN_HELP_CAVEAT`, but `set(utils.__all__) - set(cli.__all__)` on disk found FOUR missing (also `add_github_throttle_args`, `configure_github_throttle_from_args`); fixing only the named subset re-triggers the audit; (2) `hephaestus.cli` has NO COMPATIBILITY.md per-package symbol table (only `logging`/`config`/`io`/`utils` do; `cli` is Stable at `COMPATIBILITY.md:53` via the tier list), so the §G version-stamp step is N/A; (3) the existing `tests/unit/cli/test_utils.py::test_cli_all_lists_framework` is a weak membership spot-check (5 named symbols `in cli.__all__`) that stayed green despite the gap — strengthen to a `set(utils.__all__) - set(cli.__all__)` empty-assertion + re-export identity check. Reviewer risks flagged: throttle re-exports may read as scope-creep (defended as same defect class); the `test_import_surface.py`/`test_automation_boundary.py` no-regression claim was reasoned from `utils.py` pulling only stdlib + `hephaestus._version_lookup`, but NOT run. Not executed in CI |
