...
### Phase 15: "N Identical Copies" — DIFF Them First, Inject the Behavior-Bearing Variants (NEW in v1.10.0, PLANNING-ONLY)
...
*own framing* ("identical copies", "optional, can simplify") misleads the design. It is a
sibling of Phase 13 (stale duplicate *count*) and Phase 14 (patched-seam extraction); this one
focuses on the *byte-for-byte-identical* claim and on **injecting** the differences as helper
parameters.

#### 15a. Diff the supposed duplicates byte-for-byte BEFORE designing the helper signature
...
#### 15b. Preserve each caller's behavior by INJECTING the variants as parameters
...
#### 15c. A third "optional, can simplify" near-duplicate is a prompt to CLASSIFY, not to change
...
#### 15d. Flag the naming divergence: issue name vs local module convention
...
#### 15e. Assumptions a reviewer/implementer must verify on disk
...
- **The helper name** (`discover_prs_simple` vs the issue's `_discover_prs_simple`) — see 15d.
...
- **v1.10.0** (2026-06-20, PLANNING-ONLY, **unverified**): Added Phase 15 — DRY-extraction issue claims "N identical copies" that are NOT byte-identical. DIFF the supposed duplicates before designing the helper signature (#1380's two `_discover_prs` differed in a missing-PR log line's level+wording AND in the delegated `find_pr_for_issue` strategy set). Preserve each caller's behavior by INJECTING the variants: pass `self._find_pr_for_issue` as `find_fn` and the log action as an optional `on_missing: Callable[[int], None]` callback, rather than hardcoding one copy. Classify the third "(optional, can simplify)" near-duplicate (`CIDriver._discover_prs`, which feeds downstream dedup/`shared_pr_issues` fan-out/bot-PR-union) as an intentional variant — cross-reference comment, do NOT consolidate. Flagged the issue-name vs module-convention naming divergence (`_discover_prs_simple` vs underscore-free `discover_prs_simple`). Added 2 Failed Attempts rows. Captured from planning ProjectHephaestus issue #1380; NOT executed end-to-end (no code, no tests, no CI). Numbered Phase 15 / v1.10.0 after rebasing onto two concurrent same-day amends (#1381 Phase 13/v1.8.0, #1383 Phase 14/v1.9.0) that landed first. Prior v1.9.0 snapshot archived to history.
