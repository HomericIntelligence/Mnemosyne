...
- "The issue says X is broken but the file already does X — is it already fixed?"
- "A prior PR already landed the migration this issue describes — what's left?"
...
### Phase 19: An issue's premise can already be RESOLVED by a prior PR the author didn't know about — grep the current tree before planning the migration (NEW in v1.15.0, PLANNING-ONLY — ProjectHephaestus #1433)

> **Verification — `unverified` (planning-only):** This phase was captured from a PLANNING-ONLY
> session — the plan was produced and reviewed, but **NO code was written, NO tests were run, and NO
> CI executed**. Every claim about residual scope is a planning judgment, not a verified result.
> (Sibling to the planning-only Phases 10/13/14/15.) The rest of the skill remains `verified-ci`,
> except the planning-only Phases 10/13/14/15/19 and the verified-local Phases 16/12d/17/18.

This is the **before-you-plan sibling** of Phase 12a (stale "Evidence:" file lists) and Phase 13
(stale "N byte-for-byte identical" counts). Those phases warn that an issue's *enumerated* claims go
stale as prior PRs partially resolve them; Phase 19 extends the same staleness to the issue's
**core PREMISE** — the broken state the whole issue is predicated on may **already be fixed**.

The #1433 task asked to replace a bare `subprocess.run(["gh", ...])` wrapper `_gh()` in
`hephaestus/github/severity_label.py` with the circuit-breaker-protected `gh_call`. The issue body
described `_gh()` as still calling `subprocess.run` directly. Grepping the **current** tree before
planning flipped the entire scope:

```bash
# Read the file the issue says is broken, and trace when the "broken" literal last changed:
git log -S 'gh_call(list(args)' -- hephaestus/github/severity_label.py   # → prior PR #1520
grep -n 'def _gh\|subprocess.run\|gh_call' hephaestus/github/severity_label.py
#   → def _gh(...):  return gh_call(list(args), check=True).stdout   ← migration ALREADY landed
```

The core `subprocess.run → gh_call` migration the issue described had **already landed in a prior
PR (#1520)** that the issue author was unaware of. On disk, `_gh()` already read
`return gh_call(list(args), check=True).stdout`. The honest re-scope was therefore **not** "do the
migration" (a no-op / re-doing landed work) but "residual cleanup": the now-redundant one-line `_gh()`
wrapper exists only to adapt `gh_call`'s `CompletedProcess` return into a `str` via `.stdout`, so it
is a DRY/consistency smell to inline at its 3 call sites and delete.

#### 19a. Grep the CURRENT tree (and `git log -S`) before planning a migration — the fix may already be in

The issue BODY captures the broken state **at creation time**, exactly like the `Evidence:` section
(Phase 12a). Before scoping any migration/fix, read the named file and run
`git log -S '<the literal the issue says is broken>' -- <file>` to see whether a later PR already
changed it. If it has, **say so in the plan** — state plainly that the core ask is already satisfied
by PR #N and describe only the residual cleanup (if any) — rather than planning a no-op or silently
re-implementing landed work, which wastes a review cycle and risks a merge conflict against the
already-landed change.

#### 19b. Residual cleanup: a return-type-adapter wrapper is a DRY smell once the underlying call is canonical — but repoint the patched seams first

Once `gh_call` is the canonical call, a thin wrapper kept **only** to adapt its return type (here
`.stdout` extraction, turning a `CompletedProcess` into a `str`) is redundant. Inline it at its call
sites and delete it. The one prerequisite is the Phase 14 patched-seam check: a wrapper patched in
tests cannot simply vanish.

```bash
# Before deleting _gh, find every test that patches it as a seam:
grep -rn 'patch.object(.*"_gh"\|patch(".*severity_label._gh"' tests/
```

Each `patch.object(mod, "_gh", ...)` must be **repointed to patch the canonical `gh_call`** instead —
and crucially the mock's return value must change from a **string** (what `_gh()` returned) to a
**`CompletedProcess`** (what `gh_call` returns), since the inlined call sites now read `.stdout`
themselves. A patched seam that still returns a bare string after the repoint would make the call
sites read `.stdout` off a `str` and break.
...
| (PLANNING #1433) Plan the `subprocess.run → gh_call` migration the issue body described | The issue said `_gh()` in `hephaestus/github/severity_label.py` still called `subprocess.run(["gh", ...])` directly; planning would have re-implemented that migration | A prior PR #1520 — which the issue author did not know about — had ALREADY landed the core migration; on disk `_gh()` already read `return gh_call(list(args), check=True).stdout`. Planning it would have been a no-op / re-doing landed work, risking a conflict against the landed change | The issue's core PREMISE goes stale exactly like its "Evidence:" section (Phase 12a) and "N identical" count (Phase 13). Before planning a migration, READ the file and `git log -S '<the literal the issue says is broken>' -- <file>`; if the fix already landed, RE-SCOPE HONESTLY in the plan (state it's done in PR #N, describe only residual cleanup) instead of planning a no-op |
| (PLANNING #1433) Delete the redundant `_gh()` wrapper without checking patched test seams | Once `gh_call` was canonical, `_gh()` existed only to adapt the `CompletedProcess` return into a `str` via `.stdout` — a DRY smell — so deleting it looked safe | If tests `patch.object(mod, "_gh", ...)` (returning a string), deleting `_gh` breaks every patch target; and even after repointing, a mock still returning a bare string breaks the inlined call sites that now read `.stdout` off a `CompletedProcess` | Apply Phase 14: grep `patch.object(.*"_gh"` BEFORE deleting; repoint each seam to patch the canonical `gh_call` and return a `CompletedProcess` (not a string), then inline `_gh` at its 3 call sites |
...
| ProjectHephaestus | #1433 | Phase 19 — an issue's PREMISE was already RESOLVED by a prior PR the author didn't know about: #1433 asked to migrate `_gh()` in `hephaestus/github/severity_label.py` from bare `subprocess.run(["gh", ...])` to the circuit-breaker `gh_call`, but prior PR #1520 had ALREADY landed it (`_gh()` already read `return gh_call(list(args), check=True).stdout`). Re-scoped from "do the migration" to "residual cleanup": inline + delete the now-redundant `.stdout`-adapter wrapper at its 3 call sites after repointing `patch.object(_, "_gh")` seams to patch `gh_call` returning `CompletedProcess` | ⚠️ **unverified — planning only, NOT executed** (no code, no tests, no CI) |
...
`refactoring`, `dry-principle`, `helper-methods`, `radiance`, `test-fakes`, `validation-wrappers`, `layout-kernels`, `tdd`, `code-quality`, `python`, `pytest`, `private-modules`, `test-structure`, `git-signing`, `importlib-metadata`, `srp`, `extract-method`, `lru-cache`, `mock-patch`, `rglob`, `dead-code-removal`, `constants`, `frozenset`, `drift`, `intentional-variant`, `core-extras-split`, `planning`, `stale-issue-body`, `inline-vs-fixture`, `remote-branch-divergence`, `hmac`, `test-helper-dedup`, `stale-duplicate-count`, `diff-before-merge`, `parameterize-defaults`, `print-summary`, `existing-dedup-home`, `patched-test-seam`, `thin-wrapper`, `free-function-extraction`, `automation-library-boundary`, `script-to-library-shim`, `shim-vs-delete`, `output-contract-preservation`, `byte-for-byte-stdout`, `granular-vs-main`, `private-import-boundary`, `tdd-green-first-inversion`, `duplicate-bearing-script`, `deprecated-api-removal`, `breaking-change`, `lazy-loader`, `lazy-imports`, `deprecation-guard-test`, `pep-562`, `all-surfaces`, `absence-guard-test`, `repo-wide-grep-gauntlet`, `compatibility-doc`, `migration-doc`, `stale-plan-anchor`, `wrong-filename-anchor`, `indirection-root`, `grep-current-state`, `verified-against-disk-not-exempt`, `log-format-constants`, `configure-cli-logging`, `title-body-mismatch`, `stale-issue-title`, `task-plan-review-pipeline`, `authoritative-source-of-truth`, `approved-plan`, `cli-log-format`, `boundary-placement`, `intentional-variant-carveout`, `grep-gauntlet`, `env-coercion`, `read-timeout-env`, `duplicate-reader-delegate`, `thin-delegate`, `unused-import`, `f401`, `i001-import-resort`, `import-time-read`, `post-edit-tree`, `ruff-check-fix`, `stale-test-path-anchor`, `relocated-guard-tests`, `no-tests-ran-false-pass`, `find-before-run`, `canonical-public-helper`, `stale-issue-premise`, `already-resolved-by-prior-pr`, `git-log-pickaxe`, `grep-before-planning`, `re-scope-honestly`, `no-op-plan-avoidance`, `return-type-adapter-wrapper`, `stdout-adapter`, `inline-redundant-wrapper`, `completed-process-vs-string`, `gh-call`, `severity-label`

## Version History

- **v1.15.0** (2026-06-30, **planning-only for the new phase / unverified**): Added Phase 19 — an issue's core PREMISE can already be (partially or fully) RESOLVED by a prior PR the issue author did not know about, so the broken state the issue is predicated on goes stale exactly like its "Evidence:" section (Phase 12a) and its "N byte-for-byte identical" count (Phase 13). Captured from PLANNING ProjectHephaestus issue #1433 ("replace the bare `subprocess.run(["gh", ...])` wrapper `_gh()` in `hephaestus/github/severity_label.py` with the circuit-breaker-protected `gh_call`"); NOT executed end-to-end (no code, no tests, no CI). The KEY finding: the issue's premise was already STALE — a prior PR #1520 (which the author was unaware of) had ALREADY landed the core `subprocess.run → gh_call` migration; on disk `_gh()` already read `return gh_call(list(args), check=True).stdout`. Re-grepping the CURRENT tree (`git log -S 'gh_call(list(args)' -- <file>` + reading the file) BEFORE planning flipped the entire scope from "do the migration the issue describes" (a no-op / re-doing landed work) to "residual cleanup — inline the now-redundant one-line `.stdout`-adapter `_gh()` wrapper at its 3 call sites and delete it for DRY/consistency." Two sub-sections: (19a) grep the CURRENT tree and `git log -S '<the literal the issue says is broken>'` before planning a migration — the fix may already be in; when it is, re-scope HONESTLY in the plan (state it landed in PR #N, describe only residual cleanup) rather than planning a no-op. (19b) a thin wrapper kept ONLY to adapt a return type (`.stdout` over the canonical `gh_call`) is a DRY/consistency smell once the underlying call is canonical — inline + delete, but ONLY after grepping `patch.object(mod, "_gh")` and repointing those seams to patch `gh_call` and return `CompletedProcess` objects (NOT strings), per Phase 14 patched-seam guidance. Added Phase 19 (sub-sections 19a/19b), 2 Failed Attempts rows, a Verified On entry, an Outcome entry, 2 trigger phrases, a description item (20), and tags. Bumped frontmatter `version` to 1.15.0; `date` stays 2026-06-30; frontmatter `verification` stays `unverified` (per the v1.10.0+ convention), with Phase 19 carrying its own inline `unverified` / planning-only label. NOT executed (no code written, no tests run, no CI). Sibling to the planning-only Phases 10/13/14/15. Prior v1.14.0 snapshot archived to history.
