...
### Phase 24: BLOCKED-PLAN for a serialized-epic thin-wrapper sub-issue whose referenced infrastructure is UNMERGED (NEW in v1.20.0, PLANNING-ONLY — ProjectHephaestus #1822)

> **Warning:** This phase is a PLANNING-ONLY heuristic. It was distilled from PLANNING
> ProjectHephaestus #1822 (`ci_driver` as a thin wrapper over new `ci`/`merge_wait` stages), sub-issue
> 13 of 14 in the strictly-serialized epic #1809. NO code was written, NO tests ran, NO CI ran, and the
> resulting BLOCKED-PLAN had not even passed plan-review at capture. Treat every recommendation as a
> hypothesis until a real serialized-epic sub-issue plan is executed end-to-end.

**The situation.** You are planning a DRY-refactor / thin-wrapper sub-issue that is one node of a
STRICTLY-SERIALIZED epic — each sub-issue carries `Depends on #<prev>`, and the epic body explicitly
forbids working two of them concurrently. The sub-issue's BODY was written AHEAD of its prerequisites and
describes future infrastructure — a new package/dir, new classes, new functions (for #1822:
`hephaestus/automation/stages/` with `ci.py` + `merge_wait.py`, plus `PipelineConfig` / `PipelineScope`
/ `run_pipeline`) — AS IF it already exists. This is the **terminal escalation of the Phase 12/17/21
stale-anchor family**: the staleness is NOT a drifted line number in an EXISTING file; it is an ENTIRE
referenced infrastructure that DOES NOT EXIST YET because a serialized dependency is unmerged.

#### 24a. Verify the dependency chain's merge state BEFORE planning

Do NOT trust the epic/sub-issue prose about what is "done." For every `Depends on #<n>` in the chain,
check the live state, and search for any PR that already ships the referenced symbols:

```bash
# Every prerequisite in the serialized chain — check LIVE state, not the epic's prose.
for n in 1811 1816 1817 1818 1819 1820 1821; do
  echo -n "#$n: "; gh issue view "$n" --repo HomericIntelligence/ProjectHephaestus --json state --jq .state
done
# Does ANY open/merged PR already introduce the referenced infra symbols?
gh pr list --repo HomericIntelligence/ProjectHephaestus --state all \
  --search "PipelineConfig in:body OR run_pipeline in:body OR stages in:title" \
  --json number,title,state,headRefName
```

If every prerequisite is OPEN and no PR ships the symbols, the infrastructure is genuinely absent.

#### 24b. Grep the CURRENT tree for every referenced symbol / dir / file

Grep proves ABSENCE (it never proves a symbol WILL exist with a given shape). Confirm the referenced
package/dir does not exist, the referenced symbols are nowhere, and the monolith you are meant to thin
STILL holds the full body:

```bash
# The referenced package/dir does not exist (exclude __pycache__ to avoid false hits).
find hephaestus/automation -type d -name stages -not -path '*/__pycache__/*'   # → (empty)
# The referenced pipeline symbols are nowhere in the tree.
grep -rlE "PipelineConfig|PipelineScope|run_pipeline" hephaestus/automation     # → (no matches)
# The monolith you are asked to thin STILL holds the full body (nothing extracted yet).
grep -n "class _CIDriverCore" hephaestus/automation/ci_driver.py                # → still the 2585-line monolith
```

#### 24c. If prerequisites are UNMERGED and the symbols/dirs are ABSENT — produce a BLOCKED-PLAN, not a fabricated plan

A plan that describes wiring `ci_driver` over `PipelineScope(ci, merge_wait)` when none of that exists is
NOT a plan — it is fabrication. It fails on three axes simultaneously:

1. **It fails review the instant a reviewer greps the tree** — the referenced dir/symbols are absent, so
   every "modify `stages/ci.py`" step points at a file that does not exist.
2. **It is impossible to implement** — the implementer has nothing to wrap; the thin-wrapper target is a
   monolith, not the two stage modules the plan assumes.
3. **It would DUPLICATE the deliverables of the unmerged prerequisite PRs and VIOLATE the epic's
   never-work-two-concurrently serialization contract** — fabricating `stages/ci.py` here is doing
   #1811/#1816's job inside #1822's PR, and it will diverge from what those PRs actually ship.

#### 24d. A BLOCKED-PLAN MUST contain four things

- **(a) PROOF of the blocked state.** Embed the exact commands from 24a + 24b and their empty/negative
  output verbatim. The greps and `gh` state checks are the evidence; do not paraphrase them.
- **(b) An explicit REFUSAL to fabricate/duplicate.** State plainly that the plan will NOT create
  `stages/`, `PipelineConfig`, `PipelineScope`, or `run_pipeline` inside this PR, because those are the
  deliverables of the unmerged prerequisites and doing so violates the serialization contract.
- **(c) A FROZEN deferred design.** Write the concrete refactor design (how `ci_driver` becomes a thin
  wrapper once the stages exist) so it is turnkey the moment the chain unblocks — but clearly marked as
  DEFERRED / not-yet-actionable.
- **(d) A mandatory RE-GROUNDING GATE.** Define a gate to run WHEN the last prerequisite (#1821) merges:
  re-grep the CURRENT (now-merged) tree for the REAL symbol names and locations. The issue body's
  `file:line` anchors and symbol names (`PipelineScope(ci, merge_wait)`, `_wait_for_pr_terminal`, exact
  method names) are pre-implementation GUESSES written before the prerequisites existed — they are NOT
  verified fact and will drift from what actually merges.

```text
# Re-grounding gate (run only after the LAST prerequisite merges):
git fetch origin && git checkout main && git pull --ff-only
find hephaestus/automation -type d -name stages -not -path '*/__pycache__/*'   # must now EXIST
grep -rn "class PipelineScope" hephaestus/automation/                          # capture the REAL signature
grep -rn "def run_pipeline"    hephaestus/automation/                          # capture the REAL entrypoint
# Re-derive EVERY file:line anchor and symbol name from the merged tree.
# Discard the issue body's guessed anchors; plan against what actually exists.
```

#### 24e. Honesty gate

Label the plan `unverified`: no code ran, no CI ran, and the BLOCKED-PLAN itself has not passed
plan-review. A blocked-plan is the HONEST deliverable when the prerequisites are unmerged — it is not a
failure to "produce a plan," it is the only reviewable, non-fabricated output available.
...
| (PLANNING #1822) FABRICATE a plan that wires `ci_driver` over the referenced `stages/` package + `PipelineConfig`/`PipelineScope`/`run_pipeline` as if they existed | The sub-issue's body (issue 13/14 of serialized epic #1809) described the new infrastructure in prose AS IF present, so a "modify `stages/ci.py`, wrap with `PipelineScope(ci, merge_wait)`" plan looked writable | NONE of it exists on the current tree — `find hephaestus/automation -type d -name stages` (excl. `__pycache__`) is empty, `grep -rlE "PipelineConfig\|PipelineScope\|run_pipeline" hephaestus/automation` finds nothing, `ci_driver.py` still holds the full monolith `class _CIDriverCore`, and every prerequisite #1811/#1816/#1817/#1818/#1819/#1820/#1821 is OPEN. Such a plan (a) FAILS review the instant a reviewer greps the tree, (b) is impossible to implement (there is nothing to wrap), and (c) would DUPLICATE the unmerged prerequisites' deliverables and VIOLATE the epic's never-work-two-concurrently serialization contract | This is the terminal escalation of the Phase 12/17/21 stale-anchor family — the staleness is an ENTIRE ABSENT infrastructure, not a drifted line. VERIFY the dependency chain's merge state (`gh issue view <n> --json state`) and grep the CURRENT tree for every referenced symbol/dir BEFORE planning; if unmerged + absent, produce a BLOCKED-PLAN (prove the blocked state with the exact greps + `gh` checks; refuse to fabricate/duplicate; freeze the deferred design turnkey; define a re-grounding gate that re-greps the merged tree for REAL symbol names, since the issue body's `PipelineScope(ci, merge_wait)`/`_wait_for_pr_terminal` anchors are pre-implementation guesses) — Phase 24 |
...
| ProjectHephaestus | #1822 / epic #1809 | Phase 24 — PLANNING a `ci_driver` thin-wrapper sub-issue (13/14) of a STRICTLY-SERIALIZED epic whose body references infrastructure (`hephaestus/automation/stages/` with `ci.py`+`merge_wait.py`, `PipelineConfig`/`PipelineScope`/`run_pipeline`) that DOES NOT EXIST YET because prerequisites #1811/#1816/#1817/#1818/#1819/#1820/#1821 are all OPEN. Verified absence on the current tree (`find ... -type d -name stages` empty; `grep -rlE "PipelineConfig\|PipelineScope\|run_pipeline"` no matches; `ci_driver.py` still the monolith `class _CIDriverCore`) and produced a BLOCKED-PLAN — prove the blocked state with the exact greps + `gh` state checks, refuse to fabricate/duplicate the unmerged deliverables (would violate the epic's never-work-two-concurrently contract), freeze the deferred thin-wrapper design turnkey, and define a re-grounding gate that re-greps the merged tree because the issue body's `PipelineScope(ci, merge_wait)`/`_wait_for_pr_terminal` anchors are pre-implementation guesses | ⚠️ **unverified — planning only, NOT executed** (no code, no tests, no CI — the BLOCKED-PLAN had not passed plan-review at capture) |
