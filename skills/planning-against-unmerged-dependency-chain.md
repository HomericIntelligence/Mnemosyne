...
description: "Plan dependent work against explicit predecessor interfaces when required code remains on open branches; track assumptions and continue independent work."
...
tags:
  ...
  - loop-runner
  - pipeline-epic-1809
history-source: "https://github.com/HomericIntelligence/Mnemosyne/blob/e98a4da5d67f0766bc6b4bfaed1ab399fca90e9f/skills/planning-against-unmerged-dependency-chain.history"
...
1. **Verify the dependency state — do not assume it.** When an issue says
   ...
   **OPEN**) and `ls <the-package-dir>` (`hephaestus/automation/pipeline/` exited 2 —
   the package is absent). If the surface does not exist yet, label proposed paths and
   interfaces as conditional assumptions. Promote them to implementation facts only
   when actual source confirms them. If a predecessor is merged,
   use `planning-dependent-issue-unverified-upstream` (read the real code, delete
