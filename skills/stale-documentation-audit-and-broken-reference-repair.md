...
- An issue names ONE stale doc value (e.g. "the Stack Components table was wrong") — one stale value predicts siblings; audit EVERY claim category in the file (image tags, scrape targets AND ports, retention, intervals, directory tree, documented commands) in one pass, building a claim / actual / verdict table with `file:line` evidence on both sides; record the claims that check out clean too ("verified, no change" is a finding the reviewer needs)
- A doc table describes a system architecture that no longer exists (e.g. per-service direct Prometheus scrape jobs after the system moved to an exporter-sidecar model with upstream URLs in env vars) — check whether the documented architecture still exists BEFORE correcting its details; rewrite the table to the current architecture instead of patching individual ports/hosts row by row
- Docs restate pinned container image tags (or `:latest` while compose pins versions) that Dependabot keeps bumping — add a version-agnostic drift test that parses BOTH the doc's markdown table (regex) and `docker-compose.yml` (`yaml.safe_load`) and asserts containment; never hardcode version numbers in the test (each bump would re-break it); exclude locally-built services (`"image" not in spec`) and profile-gated dev services (`spec.get("profiles")`)
...
# ── FULL-FILE CLAIM AUDIT (one stale value predicts siblings) ─────────────────
# Issue names one stale table? Audit EVERY claim category in one pass:
grep -nE "image:|:latest" docker-compose.yml                 # actual pinned images
grep -nE "job_name|targets:" configs/prometheus.yml          # actual scrape jobs
grep -n "retention_period\|scrape_interval" configs/*.yml    # actual retention/interval
grep -nE "latest|localhost:[0-9]+|[0-9]{2,5}" CLAUDE.md      # documented claims
# Build a claim / actual / verdict table; record OK verdicts too.
# Before patching a table's values, confirm its ARCHITECTURE still exists
# (direct scrape jobs may have been retired for an exporter sidecar).
# Parameterized just recipes ('logs SERVICE:') escape grep '^[a-z-]*:' —
# grep the recipe NAME itself when cross-checking documented commands.
...
| Auditing only the value the issue named | ProjectArgus #103 fixed just the Stack Components table | Sibling claims had drifted worse — the scrape-targets table described a retired direct-scrape architecture and the repo tree missed 6+ entries | One stale doc value predicts siblings; audit every claim category in the file in one pass with a claim/actual/verdict table |
| Patching individual ports in an obsolete scrape table | Considered correcting `localhost:4646` → `172.20.0.1:4646` row by row | The direct-scrape rows (agamemnon/nestor/nats jobs) no longer existed as Prometheus jobs at all — the system had moved to an exporter sidecar with upstream URLs in compose env vars | Check whether the documented architecture still exists before correcting its details; rewrite the table to the current architecture |
| Hardcoding version numbers in a doc drift test | Considered asserting `"v2.54.1" in CLAUDE_MD` | Every Dependabot image bump would require a test edit — recreating the drift problem one level down | Parse both files (doc table via regex, compose via `yaml.safe_load`) and assert containment; the test stays version-agnostic |
| `grep "^[a-z-]*:" justfile` to enumerate recipes for a command cross-check | Pattern anchored to name-then-colon | Parameterized recipes (`logs SERVICE:`) have a space before the colon and escape the pattern — the documented `just logs` looked phantom | Grep the recipe NAME itself (`grep -n "logs" justfile`) or allow parameters: `grep -E "^[a-z-]+( [A-Z_]+)*:"` |
...
### Version-agnostic compose-image / scrape-target drift test (unittest + pyyaml)

```python
ROOT = Path(__file__).resolve().parent.parent
CLAUDE_MD = (ROOT / "CLAUDE.md").read_text()

def test_documented_images_match_compose(self):
    documented = dict(re.findall(r"\|\s*(\S+)\s*\|\s*(\S+:\S+)\s*\|", CLAUDE_MD))
    for service, spec in self.compose["services"].items():
        if "image" not in spec or spec.get("profiles"):
            continue  # locally built or dev-only
        self.assertIn(spec["image"], documented.values())

def test_documented_scrape_targets_exist_in_prometheus_config(self):
    real_targets = {t for job in prom["scrape_configs"]
                    for sc in job.get("static_configs", []) for t in sc.get("targets", [])}
    section = CLAUDE_MD.split("## Scrape Targets")[1].split("##")[0]
    for cell in set(re.findall(r"\|\s*[\w.-]+:\d+\s*\|", section)):
        self.assertIn(cell.strip("| ").strip(), real_targets)
```

No version literals anywhere — a Dependabot bump to `docker-compose.yml` only fails the test
when the doc was not updated to match, which is exactly the drift being guarded.
...
| ProjectArgus | Issue #235 (planning artifact, **unverified**) | Full-file CLAUDE.md audit vs `configs/prometheus.yml` / `configs/loki.yml` / `docker-compose.yml`: 3 stale claim groups (all-`:latest` image table vs pinned tags; scrape-targets table describing a retired direct-scrape architecture after exporter-sidecar migration; repo tree missing 6+ entries) and 4 clean claims (15s interval, 720h retention, all 8 documented just recipes). Audit greps executed locally; version-agnostic drift test authored but not run end-to-end. Source of triggers 27–29 |
