...
- `git rebase` prints "warning: skipped previously applied commit" (distinct from "dropping") after a Wave-0 hotfix cherry-picked the PR's exact commit to main — after rebase, both `git log origin/main..HEAD` and `git diff origin/main...HEAD` are empty; this is FULL SUBSUMPTION → close the PR immediately: `gh pr close <N> --comment "Superseded: the change this PR added was already landed on main via #<hotfix>. Closing as subsumed."` (do NOT force-push an empty branch)
...
# If empty AND git diff origin/main...HEAD is also empty: PR is FULLY SUBSUMED — close it:
#   gh pr close <N> --comment "Superseded: the change this PR added was already landed on main via #<hotfix>. Closing as subsumed."
# Note: "warning: skipped previously applied commit" = Wave-0 hotfix cherry-pick (CLOSE)
#       "dropping <sha> -- patch contents already upstream" = normal rebase dedup (CLOSE too)
