...
| `git reset --hard origin/<branch>` (per-PR) | Use a fresh `git checkout -B wt-<pr> origin/<branch>` per PR — a clean clone needs no reset at all |
| Force-push auto-resolved rebase onto a PR you did NOT author | The auto-mode classifier can deny it as "Git Destructive" — get explicit user authorization FIRST, then after pushing verify each PR's feature diff is preserved (`git diff --name-only <merge-base>..HEAD` minus infra/doc files, before vs after) |
...
| 9 sibling PRs (shared-infra fix sync, 2026-06-01) — corrected `scripts/verify-branch-protection.sh` / branch-protection-drift check landed on main; sibling PRs each carried their own stale partial copy (`>= 1` vs main's `== 0`); take-theirs rebase produced hybrid/stale trees; reliable fix = overwrite file with `git show origin/main:path > path` + sha-level grep verify; two-layer required-checks reframe (markdownlint/docker-build NOT required, branch-protection-drift was the gate); `git reset --hard` and `git worktree remove --force` blocked; force-push to non-authored PRs gated by "Git Destructive" classifier | verified-local; drift check `conclusion=success` on multiple rebased HEADs, full required suite still running at session end |
