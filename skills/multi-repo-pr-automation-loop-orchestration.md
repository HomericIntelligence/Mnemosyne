...
tags:
  ...
  - terminal-pr-outcome
  - bounded-org-source
  - repository-local-numeric-scope
...
| **Outcome** | Driver hardening established honest live-state reporting, bounded per-repo concurrency, target-repo-scoped direct pipelines, and direct-PR terminalization. v1.7.0 adds the missing scope invariant: an org iterator may stream discovery, but a numeric issue or PR is repository-local. Reject `--org` plus `--issues`/`--prs` unless exactly one `--repos REPO` is supplied; never materialize the organization to choose a target and never silently route to its first repository. |
...
| **Latest Update** | v1.7.0 adds the bounded-org/direct-numeric-scope rule, verified locally in ProjectHephaestus PR #2436: reject ambiguous `--org ORG --issues ...` and `--org ORG --prs ...` before `_gh_list_repos`; also reject multiple `--repos` for one numeric target. |
...
- An org-wide run accepts repository-local numeric `--issues` or `--prs` without a single concrete repository, or an implementation is tempted to enumerate the whole organization and select its first repository to recover compatibility.
...
Keep an org iterator bounded when a direct numeric scope is requested:

```bash
# Valid: numeric issue/PR identity is scoped to the one named repository.
uv run hephaestus-automation-loop --org HomericIntelligence --repos Hephaestus --issues 2399
uv run hephaestus-automation-loop --org HomericIntelligence --repos Hephaestus --prs 2436

# Reject before discovery: these numbers are repository-local, not org-global.
uv run hephaestus-automation-loop --org HomericIntelligence --issues 2399
uv run hephaestus-automation-loop --org HomericIntelligence --repos Hephaestus,Odysseus --prs 2436
```
...
**Bounded organization discovery cannot infer a direct numeric target (v1.7.0 / PR #2436).**
Repository names are globally meaningful within an organization; issue and PR numbers are not. A lazy,
paged organization source is correct for an unscoped sweep, but `--org ORG --issues N` and `--org ORG --prs N`
do not identify one repository. Never restore a legacy compatibility path by materializing all repositories
and passing `repos[0]` into the direct source: that reintroduces O(org) memory/API work and silently processes
the alphabetically first repository.

Require exactly one explicit `--repos REPO` whenever `--issues` or `--prs` is present. Reject both the
zero-repository and multi-repository forms before any `_gh_list_repos` / organization API call. Preserve the
resettable, paged factory only for the unscoped `--org` sweep. The regression contract has three parts:

1. `--org ORG --issues N` rejects and does not call the materializer.
2. `--org ORG --prs N` rejects and does not call the materializer.
3. `--repos A,B --issues N` (and the equivalent PR form) rejects rather than letting a downstream
   `config.repos[0]` choose the target.
...
| --------- | ---------------- | --------------- | ---------------- |
| Materialize an organization to resolve `--issues` / `--prs` (v1.7.0) | Kept a compatibility branch for `--org ORG --issues N` / `--prs N`: listed every repository and passed the resulting list to direct seeding | It defeated the paged source's boundedness guarantee and direct seeding consumed `config.repos[0]`, silently targeting the first repository rather than the owner of N | Numeric issue and PR identities are repository-local. Require exactly one `--repos REPO` before direct seeding; reject zero or multiple repositories without calling the org materializer |
...
| #2436 | #2399 | bounded organization discovery and lease safety: organization repositories are streamed through a resettable paged factory; direct numeric scope now requires exactly one explicit repo rather than materializing the organization or implicitly selecting its first repository. Verified locally: loop-runner suite 76 passed, ruff and mypy clean. Project CI is pending. |
...
| ProjectHephaestus | Bounded org source + direct numeric scope | PR #2436 / issue #2399: two independent re-reviews caught an eager `--org` plus numeric-scope compatibility branch after the primary bounded-source changes. The correction rejects ambiguous zero/multi-repo numeric scope before `_gh_list_repos`; loop-runner tests (76), ruff, and mypy passed locally. CI had not yet completed when recorded. |
